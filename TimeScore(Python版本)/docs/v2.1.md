# 🎯 TimeScore iOS Develop Guide V2.1

## 📊 迁移状态

| 模块 | 状态 | 文件路径 |
|------|------|----------|
| **CoreData Schema** | ✅ 完成 | `TimeScore/Core/CoreDataStack.swift` |
| **数据访问层** | ✅ 完成 | `TimeScore/Core/CoreDataManager.swift` |
| **积分计算系统** | ✅ 完成 | `TimeScore/ViewModels/ScoringViewModel.swift` |
| **精力管理系统** | ✅ 完成 | `TimeScore/ViewModels/EnergyViewModel.swift` |
| **行为系统** | ✅ 完成 | `TimeScore/ViewModels/BehaviorViewModel.swift` |
| **积分兑换系统** | ✅ 完成 | `TimeScore/ViewModels/ExchangeViewModel.swift` |
| **可视化系统** | ✅ 完成 | `TimeScore/ViewModels/VisualizationViewModel.swift` |
| **首页界面** | ✅ 完成 | `TimeScore/Views/Home/HomeView.swift` |
| **详细记录界面** | ✅ 完成 | `TimeScore/Views/Record/DetailRecordView.swift` |
| **成功反馈弹窗** | ✅ 完成 | `TimeScore/Views/Common/SuccessOverlayView.swift` |
| **行为选择界面** | ✅ 完成 | `TimeScore/Views/Record/BehaviorPickerView.swift` |
| **数据洞察界面** | ✅ 完成 | `TimeScore/Views/Visualize/InsightsView.swift` |
| **心愿兑换界面** | ✅ 完成 | `TimeScore/Views/Redeem/WishlistView.swift` |
| **TabView 架构** | ✅ 完成 | `TimeScore/Views/MainTabView.swift` |
| **单元测试** | 🔄 待实现 | `TimeScoreTests/` |
...
#### 字体与排版
- **动态字重字体对比**：积分大数字使用SF Pro Rounded Bold（48pt），标签使用SF Pro Text Regular（16pt），形成强烈大小/粗细对比。
- **截断文本+展开控制**：行为备注（notes）在历史列表中采用单行截断，详情页显示全文，控制信息密度。

#### 色彩系统
- **语义化单色调和**：以黑白灰为基底，功能色严格对应状态（绿#4CAF50正面行为、蓝#2196F3恢复、红#F44336负分）。
- **深色底部导航栏**：底部Tab栏采用深色（#1C1C1E）与内容区白色形成强分隔，选中态使用品牌绿色点缀。
...
- **HomeView（行为记录主屏）**：
  ...
  - **中部输入卡**：行为等级Picker（S/A/B/C/D/R药丸分段器）,点击等级展开具体所属行为，显示时长Stepper、心情1-5星评分、备注TextField。
